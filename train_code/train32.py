...
n=do_norm(torch.ones(16).type(torch.float32).cuda())
dataH=sio.loadmat(f'{data_dir}/H{task}.mat')["H"]
index=sorted(range(len(data)),key=lambda x:md5(str(x).encode()).hexdigest())
dataH=dataH[index][:900]
dataH=dataH.transpose(0,1,3,2).conj()
dataH=np.stack([dataH.real,dataH.imag],-1).reshape(-1,16).astype(np.float32)
bg=do_norm(torch.from_numpy(dataH).cuda())
bg=cal_nn(n.unsqueeze(0).repeat(len(bg),1),bg)
bg=do_norm(bg)
model.encoder.background[:len(bg)]=bg
